...
if(!eval { require JSON::XS; 1 } && !eval { require Cpanel::JSON::XS; 1 } && !eval { require JSON::PP; 1 })
{
    plan skip_all => 'No module of the following available: JSON::XS, Cpanel::JSON::XS and JSON::PP';
}
...
{   my ($schema, $test, $xml, $expected_json, %args) = @_;
    ...
    SKIP: {
        skip "No bignum implementation with " . ref($json_serializer), 1 if $args{need_bignum} && !$json_serializer->can('allow_bignum');
	...
	is $got_json, $expected_json, "json serialization (" . ref($json_serializer) . ") for $test";
    }
}
...
SKIP: foreach my $json_module ('JSON::PP', 'JSON::XS', 'Cpanel::JSON::XS')
{
    skip "Skip tests with $json_module", 1
	if !eval qq{ require $json_module; 1 };
